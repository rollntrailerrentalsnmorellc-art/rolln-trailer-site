...
export default async function PaymentsPage() {
  ...
  const bookings = (data ?? []) as BookingPayment[];
  const recordedRevenue = bookings.reduce((sum, booking) => sum + (booking.amount_paid_cents ?? 0), 0);
  ...
  return (
    <main><section><div className="container">
      ...
      {error && <div className="notice">Unable to load payments: {error.message}</div>}
      {!error && bookings.length === 0 && <div className="panel"><h2>No payment records yet</h2></div>}
