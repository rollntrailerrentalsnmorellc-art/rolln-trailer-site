...
export async function GET(req:NextRequest){
 ...
 const {data,error}=await supabase.from('bookings').select('id').eq('trailer_id',trailerId)
  .in('status',['pending_payment','confirmed','active']).lt('pickup_at',end.toISOString()).gt('return_at',start.toISOString()).limit(1);
